...
class ErrorDetectorConfig(BaseConfig):
    ALGO_NAME = "error_detector"

    def train_config(self):
        """
        BC algorithms don't need "next_obs" from hdf5 - so save on storage and compute by disabling it.
        """
        super(ErrorDetectorConfig, self).train_config()
    ...
    def observation_config(self):
        ...
        """
        super(ErrorDetectorConfig, self).observation_config()
        self.observation.modalities.subgoal_obs.low_dim = []
        self.observation.modalities.subgoal_obs.rgb = ["rgb"]                      # specify rgb image subgoal observations for agent to predict
        self.observation.modalities.subgoal_obs.depth = []
        self.observation.modalities.subgoal_obs.scan = []
        self.observation.modalities.subgoal_obs.do_not_lock_keys()
    ...
    def all_obs_keys(self):
        ...
        return sorted(tuple(set([
            obs_key for group in [
                ...
                self.observation.modalities.subgoal_obs.values(),
            ]
